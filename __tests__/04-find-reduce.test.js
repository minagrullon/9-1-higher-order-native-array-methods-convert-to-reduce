...
describe('findPinkElephantsByTimestreet()', () => {
  ...
  test('should use the `.reduce()` method', () => {
    const hasReduce = !!findPinkElephantsByTimestreet
      .toString()
      .match(/\.reduce(\s*\(|\()/g);
...
describe('findFirstSongUnderThreeMinutes()', () => {
  ...
  test('should use the `.reduce()` method', () => {
    const hasReduce = !!findFirstSongUnderThreeMinutes
      .toString()
      .match(/\.reduce(\s*\(|\()/g);
    expect(hasReduce).toBeTruthy();
  });
...
describe('findFirstTitleTrack()', () => {
  ...
  test('should use the `.reduce()` method', () => {
    const hasReduce = !!findFirstTitleTrack
      .toString()
      .match(/\.reduce(\s*\(|\()/g);
    expect(hasReduce).toBeTruthy();
  });
