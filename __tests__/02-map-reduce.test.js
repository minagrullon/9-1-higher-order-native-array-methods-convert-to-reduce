...
describe('getSongTitles()', () => {
  ...
  test('should use the `.reduce()` method', () => {
    const hasReduce = !!getSongTitles.toString().match(/\.reduce(\s*\(|\()/g);
...
describe('getSongDetails()', () => {
  ...
  test('should use the `.reduce()` method', () => {
    const hasReduce = !!getSongDetails.toString().match(/\.reduce(\s*\(|\()/g);
    expect(hasReduce).toBeTruthy();
  });
...
describe('getTitleAndArtist()', () => {
  ...
  test('should use the `.reduce()` method', () => {
    const hasReduce = !!getTitleAndArtist
      .toString()
      .match(/\.reduce(\s*\(|\()/g);
    expect(hasReduce).toBeTruthy();
  });
